...
3. Upgrade the fncm-operator on your cluster.
   ...
   ```bash
   $ cd container-samples/operator
   $ ./scripts/5.5.4_iFix1/upgradeOperator.sh -i <registry_url>/ fncm-operator:ga-554-if001 -p 'admin.registrykey' -a accept
   ```
...
4. Monitor the pod until it shows a STATUS of Running:
   ...
   ```bash
   $ kubectl logs -f deployment/ibm-fncm-operator -c operator
   ```    
