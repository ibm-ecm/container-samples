...
[ -f ./upgradeOperator.yaml ] && rm ./upgradeOperator.yaml
cp ./descriptors/operator.yaml ./upgradeOperator.yaml
...
if [[ $LICENSE_ACCEPTED == "accept" ]]; then
    ...
    sed -e "s|image: .*|image: \"$IMAGEREGISTRY\" |g" ./upgradeOperator.yaml > ./upgradeOperatorsav.yaml ;  mv ./upgradeOperatorsav.yaml ./upgradeOperator.yaml
    fi

    # Change the pullSecrets if needed
    if [ ! -z ${PULLSECRET} ]; then
        echo "Setting pullSecrets to $PULLSECRET"
        sed -e "s|admin.registrykey|$PULLSECRET|g" ./upgradeOperator.yaml > ./upgradeOperatorsav.yaml ;  mv ./upgradeOperatorsav.yaml ./upgradeOperator.yaml
    else
        sed -e '/imagePullSecrets:/{N;d;}' ./upgradeOperator.yaml > ./upgradeOperatorsav.yaml ;  mv ./upgradeOperatorsav.yaml ./upgradeOperator.yaml
    fi
    ...
    kubectl apply -f ./upgradeOperator.yaml --validate=false
