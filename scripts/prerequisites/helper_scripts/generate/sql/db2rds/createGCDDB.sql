-- **********************************************************
-- IBM FileNet Content Manager GCD preparation script for DB2RDS
-- **********************************************************
-- Usage:
...
CALL rdsadmin.create_database('${gcd_name}',32768,'UTF-8','US' );
...
-- Create bufferpool
CALL rdsadmin.create_bufferpool('${gcd_name}','${gcd_name}_1_32K',1024,'Y','Y',32768,0,32);
CALL rdsadmin.create_bufferpool('${gcd_name}', '${gcd_name}_2_32K',1024,'Y','Y',32768,0,32);

-- Create table spaces
CALL rdsadmin.create_tablespace( '${gcd_name}', '${gcd_name}DATA_TS', '${gcd_name}_1_32K', 32768, NULL, NULL, 'U', 'AUTOMATIC');
CALL rdsadmin.create_tablespace( '${gcd_name}', '${gcd_name}_TMP_TBS', '${gcd_name}_2_32K', 32768, NULL, NULL, 'T', 'AUTOMATIC');

-- Create role for the database GCDDB with the role name of FNCM
CALL rdsadmin.create_role('${gcd_name}','FNCM');

-- Create a user
CALL rdsadmin.add_user('${youruser1}','${yourpassword}',null);
CALL rdsadmin.grant_role(?,'${gcd_name}','FNCM','USER ${youruser1}','N');
CALL rdsadmin.update_db_param('${gcd_name}','LOCKTIMEOUT','30');
