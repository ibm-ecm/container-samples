...
function create_new_shared_logs_pvc(){
    ...
    PLATFORM_SELECTED=$(eval echo $(oc get fncmcluster $(oc get fncmcluster | grep NAME -v | awk '{print $1}') -o yaml | grep sc_deployment_platform | tail -1 | cut -d ':' -f 2))
    STORAGE_CLASS_NAME=$(eval echo $(oc get fncmcluster $(oc get fncmcluster | grep NAME -v | awk '{print $1}') -o yaml | grep sc_dynamic_storage_classname | tail -1 | cut -d ':' -f 2))
    SLOW_STORAGE_CLASS_NAME=$(eval echo $(oc get fncmcluster $(oc get fncmcluster | grep NAME -v | awk '{print $1}') -o yaml | grep sc_slow_file_storage_classname | tail -1 | cut -d ':' -f 2))
    FAST_STORAGE_CLASS_NAME=$(eval echo $(oc get fncmcluster $(oc get fncmcluster | grep NAME -v | awk '{print $1}') -o yaml | grep sc_fast_file_storage_classname | tail -1 | cut -d ':' -f 2))
