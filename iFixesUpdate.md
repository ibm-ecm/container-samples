...
2. Go to the downloaded container-samples.git for FileNet Content Manager V5.5.6
...
4. Upgrade the fncm-operator on your project.
   ...
   or
   $ ./scripts/upgradeOperator.sh -i <registry_url>/icp4a-operator:21.0.1 -p 'admin.registrykey' -a accept
   ```
...
2. Verify the appVersion in the global spec section matches the appVersion in the readme file for the operator interim fix you are deploying. The appVersion depends on the version of the operator and choice of eGA or interim fix images. The appVersion setting automatically defaults the image:tag or digest values for deployed containers to reflect a particular release. The following table shows the operator interim fixes and their corresponding appVersion values. 
